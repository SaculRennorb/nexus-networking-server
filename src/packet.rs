...
pub trait Packet {
	fn calculate_crc(&self) -> Checksum {
		...
		let as_u32s : &[u32] = unsafe { std::slice::from_raw_parts(_self.cast(), _self.cast::<Header>().as_ref().unwrap_unchecked().length_in_u32s as usize - 1) };  // skip the last u32, thats where the crc is going to go
...
pub const MIN_PACKET_SIZE : usize = ceil_to_u32_boundary(size_of::<Header>()) + ceil_to_u32_boundary(size_of::<Checksum>());
...
pub mod internal {
	...
	use crate::{packet_data::{internal::{Data as InternalData, DynamicData, SizedData}, Data}, util::ceil_to_u32_boundary, SessionId, UserId};

	pub const MIN_PACKET_SIZE : usize = ceil_to_u32_boundary(size_of::<ExtendedHeader>()) + ceil_to_u32_boundary(size_of::<Checksum>());
